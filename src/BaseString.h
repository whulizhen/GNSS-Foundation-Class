#pragma ident "$Id: BaseString.h 3140 2012-06-18 15:03:02Z susancummins $"
...
#ifndef GFC_BASESTRING_H
#define GFC_BASESTRING_H
...
#ifdef _WIN32
	#if _MSC_VER < 1700
        // For lower version of visual studio 2012 use gnu regex(�汾Ϊ2.7)
...
namespace gfc
{
   ...
   namespace StringUtils
   {
       ...
       NEW_GEXCEPTION_CLASS(StringException, gfc::GException);
      ...
      { std::string t(s);  return c2x(t); }
          ...
          */
      inline uint_t x2int(const std::string& s)
         throw(StringException);
          ...
          */
      inline std::string int2x(const uint_t& i)
         throw(StringException);
      ...
      inline std::string firstWord(const std::string& s, 
          ...
          */
      inline int_t numWords(const std::string& s, 
                          const char delimiter = ' ')
         throw(StringException);
...
namespace gfc
{
   ...
   namespace StringUtils
   {
      ...
      {
         std::string ascii="";
         uint_t indent = static_cast<uint_t>(tag.length()) ;
         int_t col = 0;
    ...
    unsigned count = 0;
         ...
         throw(StringException)
      {
         try
         {
            int_t spot = static_cast<int_t>(s.find(oldString, 0)) ;
            while (spot != (int_t)std::string::npos)
            {
               s.replace(spot, oldString.length(), newString);
               spot += newString.length();
               spot = static_cast<int_t>(s.find(oldString, spot)) ;
            }
            return s;
         }
         catch(std::exception &e)
		  ...
		  throw(StringException)
	  {
		  ...
		  int_t rc = regcomp(&re, pat.c_str(), REG_EXTENDED);
         ...
         throw(StringException)
      {
         try
         {
            ...
            while ((pos != std::string::npos) && (pos <= s.length()))
            {
               ...
               if (((int_t)numWords != -1) && ((int_t)wordNum == (int_t)(firstWord + (numWords-1))))
                  break;
               pos = s.find_first_not_of(delimiter, pos);
               wordNum++;
            }
                                            ...
                                            bool showPlus)
      {
         ...
         int_t i = (int_t(leng) - int_t(elen) - 4);
         if(showPlus) i--;
         ...
         throw(StringException)
      {
         try
         {
            ...
            for (int_t i = 0; i < (int_t)rv.length(); i++)
            {
               char c = rv[i];
               if (!isprint(c))
               {
